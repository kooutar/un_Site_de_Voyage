    ...
    <footer class="bg-blue-600 text-white py-6">
       ...
       <div id="modal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-50">
        <div class="bg-white rounded-lg w-full max-w-md p-6 shadow-lg relative">
            ...
            <input type='hidden' name='id_activite' value='<?php echo $activite->getID_activite(); ?>'>
            <!-- Nombre de personnes -->
                <div>
                    <label for="nbrPersonnes" class="block text-sm font-medium text-gray-700 mb-1">Nombre de personnes</label>
                    <input type="number" id="nbrPersonne" name="nbrPersonne" class="w-full border border-gray-300 rounded-lg p-2 focus:ring-blue-600 focus:border-blue-600" placeholder="Entrez le nombre de personnes" required>
                </div>
...
<script>
    const reserver=document.querySelectorAll('.resrever');
    reserver.forEach((btn)=>{
